...
Además, el análisis de los productos más comprados puede ayudar a identificar tendencias de consumo y patrones de compra a lo largo del tiempo. Esto puede ser especialmente útil para prever la demanda de ciertos productos, planificar promociones o descuentos, y optimizar la gestión de inventario.
...
En conjunto, estos hallazgos subrayan la importancia de entender las preferencias y tendencias de compra de los clientes para adaptar la oferta de productos y mejorar la experiencia de compra en el supermercado. Además, proporcionan insights valiosos para estrategias de marketing, promoción y gestión de inventario que podrían ayudar a aumentar las ventas y la satisfacción del cliente.

## ¿A qué hora se realizan más compras?

Esta pregunta es fundamental en el análisis de datos de un supermercado por varias razones. En primer lugar, entender los patrones de compra horaria proporciona información valiosa sobre el comportamiento del consumidor. Conocer las horas pico de actividad en el supermercado permite a los gerentes y propietarios optimizar la distribución de personal y recursos durante esos períodos, asegurando así una experiencia de compra más fluida y satisfactoria para los clientes.
...
```
Estas observaciones nos brindan una visión clara de los momentos críticos del día en los que se registra un mayor flujo de clientes en el supermercado. Aprovechar esta información estratégicamente puede tener un impacto significativo en la experiencia general del cliente y en la eficiencia operativa del establecimiento.
...
Además, ajustar los recursos humanos según la demanda también puede mejorar la gestión del inventario y la reposición de productos. Mantener las estanterías abastecidas y ordenadas durante los períodos de mayor afluencia ayuda a garantizar que los clientes encuentren lo que necesitan rápidamente, lo que puede aumentar las ventas y minimizar la pérdida de oportunidades de venta debido a productos agotados.
...
## ¿Cuáles son los productos menos comprados?

Identificar cuáles son los productos menos comprados en un supermercado es igualmente crucial por varias razones. En primer lugar, ofrece información valiosa sobre los productos que pueden no estar atrayendo la atención de los clientes o que tienen una demanda baja. Esto permite a los supermercados revisar su estrategia de inventario y marketing para impulsar la visibilidad y el interés en estos productos.

Además, el análisis de los productos menos comprados puede ayudar a detectar posibles problemas con la calidad, el precio o la disponibilidad de ciertos productos. Si un artículo específico está constantemente entre los menos vendidos, podría indicar la necesidad de ajustes en su presentación, precio o disponibilidad para mejorar su atractivo y aumentar su demanda.
...
ggplot(top_10_productos, aes(x = "", y = Frecuencia, fill = Producto)) +
  ...
  theme(legend.position = "right", 
        plot.background = element_rect(fill = "#ebf2ff"))
```
Al analizar los datos, se observa que el aceite virgen extra, los huevos grandes y las acelgas son algunos de los productos menos vendidos en el supermercado. Esta información proporciona una oportunidad valiosa para revisar y ajustar las estrategias de marketing y venta en relación con estos productos.

Es importante considerar si hay aspectos específicos que están afectando la demanda de estos productos, como su precio, presentación o disponibilidad en tienda. Realizar ajustes en estas áreas podría ayudar a aumentar su atractivo para los clientes y, en consecuencia, impulsar las ventas.

## ¿Relación productos más vendidos y su precio?
...
frecuencia_df <- cbind(mas_vendidos, menos_vendidos)
...
# Gráfico
library(ggplot2)
p <- ggplot(df, aes(x = producto, y = precio, size = frecuencia)) + 
...
#  geom_jitter(width = 0.2, color = "grey30") +
 # scale_size_continuous(range = c(3, 8)) +
  #theme_minimal() +
  #labs(x = "Producto", y = "Precio", size = "Frecuencia")
